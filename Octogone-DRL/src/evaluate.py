...
def evaluate(config_file, model_path, num_episodes=10, render=False, **kwargs):
    ...
    env = OctogoneEnv(
        reward_persona=config["env"]["reward_persona"],
    ...
    for i in range(num_episodes):
        ...
        while not done and num_steps < MAX_STEPS_PER_EPISODE:
            ...
            episode_metrics["distance_traveled"] += np.linalg.norm(obs[:2] - last_pos)
            last_pos = obs[:2]
            episode_metrics["unique_tiles_visited"].add(
                tuple(np.round(obs[:2]).astype(int))
            )
            episode_metrics["max_x_velocity"] = max(
                episode_metrics["max_x_velocity"], abs(obs[2])
            )
            episode_metrics["max_y_velocity"] = max(
                episode_metrics["max_y_velocity"], abs(obs[3])
            )
            episode_metrics["max_momentum"] = max(
                episode_metrics["max_momentum"], obs[5]
            )

            num_steps += 1

        if done:
            if np.linalg.norm(obs[:2] - env.goal_pos) < 20.0:
                episode_metrics["level_completed"] = 1
    ...
    aggregate_metrics = {
        ...
        "levels_completed": int(df["level_completed"].sum()),
        "mean_distance_traveled": float(df["distance_traveled"].mean()),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num_platforms", type=int, default=10, help="Number of platforms in the level"
    )
    parser.add_argument(
        "--num_spikes", type=int, default=5, help="Number of spikes in the level"
    )
    parser.add_argument(
        "--max_gap_size",
        type=int,
        default=200,
        help="Maximum gap size between platforms",
    )
